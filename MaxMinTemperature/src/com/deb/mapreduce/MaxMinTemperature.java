...
* @since 16-Jan-2014
* @package com.deb.mapreduce
* The MaxMinTemperature program finds out the date when temperature is greater than 40,
* and calls it as a Hot Day and when temperature is lower than 10, it calls it as a Cold
* Day from a given input file. It also finds all time maximum and minimum temperature.
* We write a map reduce code to achieve this, where mapper makes key value pair from the 
...
public class MaxMinTemperature {
				...
				Mapper<LongWritable, Text, Text, Text>
	{
	    	 ...
	    	 * by taking whitespace as delimiter. Now leaving the first five tokens, 6th token is taken as maxtemp and 
	    	 * 7th token is taken as mintemp. These maxtemp and mintemp are passed to the reducer.                                       
							...
							while (st.hasMoreTokens())
								{
									...
									/*
									 * After moving through 1st five fields in the input file, the 6th field has been taken 
									 * here as maxtemp and 7th field as mintemp
									 */
			...
			//Reducer
			     ...
			     * interface having four hadoop generics type Text, Text, Text, Text.
			     */
		
			public static class TemperatureReduce extends MapReduceBase implements Reducer<Text, Text, Text, Text>
									{
				...
				/*
					...
					public void reduce(Text key, Iterator<Text> values,OutputCollector<Text, Text> output, Reporter reporter) throws IOException
									...
									//Converting key to String and dumping it to mykey variable of type String
									...
									//Converting the value to string and putting it to variable dateandtemp of type String	
									String dateandtemp = values.next().toString();
									
									//Splitting dateandtemp on delimiter tab "/t" and storing data into dt variable
									String dt[]= dateandtemp.split("/t");
									
									//Setting the date
									date.set("max tempertature was on :"+dt[0]+"that is");
									
									//Converting second value in dt to float and storing it to variable temperature of type float
									float temperature =Float.parseFloat( dt[1]);
									
									//If temperature is greater than maxtemp then maxtemp is assigned that temperature.
									...
									//If temperature is greater than mintemp then mintemp is assigned that temperature.
