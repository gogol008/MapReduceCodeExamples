...
LOCATION
  ...
  LOAD DATA LOCAL INPATH "/root/Hive/data/runs_opposition.csv" INTO TABLE player_opposition_runs;
...
LOAD DATA LOCAL INPATH "/root/Hive/data/runs_opposition.json" INTO TABLE player_opposition_runs_json;

select * from player_opposition_runs_json limit 10;

Serde XML:
---------------------------------------------------------
ADD JAR /root/Hive/hivexmlserde-1.0.4.0.jar;
...
LOAD DATA LOCAL INPATH "/root/Hive/data/runs_opposition.xml" INTO TABLE player_opposition_runs_xml;
...
LOAD DATA LOCAL INPATH '/root/Hive/data/NASDAQ_daily_prices_A_sample.csv' OVERWRITE INTO TABLE nasdaq_prices; 

head /root/Hive/data/NASDAQ_daily_prices_A_sample.csv | tr ',' '\t' | python stock_mapper.py


add file /root/Hive/stock_mapper.py;

select transform (exchange1,stock_symbol,modified_date,stock_price_open,stock_price_high,stock_price_low,stock_price_close,stock_volume,stock_price_adj_close) using "python /root/Hive/stock_mapper.py" from nasdaq_prices where exchange1="NASDAQ" limit 10;


add file /root/Hive/stock_reducer.py;
...
USING 'python /root/Hive/stock_mapper.py'
AS price_type, price order by price_type) map_output
REDUCE price_type, price
USING 'python /root/Hive/stock_reducer.py'
AS price_type, price;
